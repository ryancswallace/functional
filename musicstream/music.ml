...
let rec list_to_stream (lst : obj list) : event stream =
  let rec list_to_stream_rec nlst =
    ...
    | h::t ->
      (match h with
      | Note (pit, time, vol) -> 
        fun () -> Cons (Tone (0., pit, vol), fun () -> 
          Cons (Stop (time, pit), list_to_stream_rec t))
      | Rest (time) -> 
        fun () -> Cons (Tone (0., (C, 0), 0), fun () -> 
          Cons (Stop (time, (C, 0)), list_to_stream_rec t)))
    | [] -> list_to_stream lst
...
let rec pair (a : event stream) (b : event stream) : event stream =
  let ha, hb = head a, head b in
  let ta, tb = tail a, tail b in
  let timea, timeb = time_of_event ha, time_of_event hb in
  if timea <= timeb then
    fun () -> Cons (ha, pair ta (fun () -> Cons (shift (-. timea) hb, tb)))
  else
    fun () -> Cons (hb, pair (fun () -> Cons (shift (-. timeb) ha, ta)) tb)
...
let transpose (str : event stream) (half_steps : int) : event stream =
  let transp (e : event) : event = 
    match e with 
    | Tone (time, pit, vol) -> Tone (time, transpose_pitch pit half_steps, vol)
    | Stop (time, pit) -> Stop (time, transpose_pitch pit half_steps)
  in
  map transp str
...
let canon = pair bass (pair (shift_start 2. melody) (pair 
